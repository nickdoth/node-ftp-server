var Base  = require('./fs').Base
  ...
  , stat2string = require('./stat2string');
...
Filesystem.prototype.list = function (outputFormatter, dir, cb) {
...
Filesystem.prototype.readFile = function (file, cb) {
  ...
  console.log('request target name:', target);
...
Filesystem.prototype.writeFile = function (file, cb) {
  // this.respond(cb, null, [fs.createWriteStream(this.resolve(file))])
...
Filesystem.prototype.getSize = function (file, cb) {
    ...
    // var stat = fs.statSync(this.resolve(file));
    // if(stat) {
        // cb(0, stat.size);
    // } else {
        // cb('Error statSync', 0);
